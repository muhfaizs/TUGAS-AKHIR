...
class PdfExportController extends Controller
{
    ...
    public function downloadRekamMedisAnak($id_anak)
    {
        ...
        // Security check
        if (auth()->user()->role === 'orang tua' && auth()->user()->nik_ortu !== $anak->nik_ortu) {
    ...
    public function downloadTindakanMedis(TindakanMedis $tindakan)
    {
        ...
        if (auth()->user()->role === 'orang tua' && auth()->user()->nik_ortu !== $tindakan->anak->nik_ortu) {
            abort(403, 'Unauthorized access.');
        }
    ...
    public function downloadImunisasi(Imunisasi $imunisasi)
    {
        ...
        if (auth()->user()->role === 'orang tua' && auth()->user()->nik_ortu !== $imunisasi->anak->nik_ortu) {
            abort(403, 'Unauthorized access.');
        }
