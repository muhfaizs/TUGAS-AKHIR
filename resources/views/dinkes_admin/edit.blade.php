...
@section('title', 'Edit Dinkes - SatuKIA')
@section('header_title', 'Edit Data Dinkes')
@section('header_subtitle', 'Perbarui informasi data pengguna Dinkes')

@section('content')
<div class="max-w-3xl">
    <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
        <form action="{{ route('dinkes.update', $dinkes->id) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
            
            <div class="mb-8">
                <h3 class="text-lg font-bold text-slate-800 mb-1">Informasi Dinkes</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $dinkes->name) }}" class="block w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition-colors" required placeholder="Masukkan nama lengkap Dinkes">
                </div>

                <!-- NIP -->
                <div>
                    <label for="nip" class="block text-sm font-bold text-slate-700 mb-2">NIP (Nomor Induk Pegawai)</label>
                    <input type="text" name="nip" id="nip" value="{{ old('nip', $dinkes->nip) }}" class="block w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition-colors" required maxlength="18" minlength="18" pattern="[0-9]{18}" oninput="this.value = this.value.replace(/[^0-9]/g, '');" placeholder="18 Digit NIP">
                </div>

                <!-- Nomor HP / Kontak -->
                <div>
                    <label for="phone" class="block text-sm font-bold text-slate-700 mb-2">Nomor HP</label>
                    <input type="tel" name="phone" id="phone" value="{{ old('phone', $dinkes->phone) }}" class="block w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition-colors" required placeholder="Contoh: 08123456789" oninput="this.value = this.value.replace(/[^0-9]/g, '');">
                </div>

                <!-- Email -->
                <div class="md:col-span-2">
                    <label for="email" class="block text-sm font-bold text-slate-700 mb-2">Alamat Email</label>
                    <input type="email" name="email" id="email" value="{{ old('email', $dinkes->email) }}" class="block w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition-colors" required placeholder="Dinkes@puskesmas.com">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="status" id="status" class="block w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition-colors font-medium" required>
                        <option value="aktif" {{ old('status', $dinkes->status) === 'aktif' ? 'selected' : '' }}>Aktif</option>
                        <option value="nonaktif" {{ old('status', $dinkes->status) === 'nonaktif' ? 'selected' : '' }}>Non-Aktif</option>
                    </select>
                </div>
