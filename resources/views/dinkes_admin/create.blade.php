...
@section('title', 'Tambah Dinkes - SatuKIA')
@section('header_title', 'Tambah Dinkes Baru')
@section('header_subtitle', 'Masukkan data pengguna Dinkes ke dalam sistem')

@section('content')
<div class="max-w-3xl">
    <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
        <form action="{{ route('dinkes.store') }}" method="POST" class="p-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" class="block w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition-colors" required placeholder="Masukkan nama lengkap Dinkes">
                </div>

                <!-- NIP -->
                <div>
                    <label for="nip" class="block text-sm font-bold text-slate-700 mb-2">NIP (Nomor Induk Pegawai)</label>
                    <input type="text" name="nip" id="nip" value="{{ old('nip') }}" class="block w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition-colors" required maxlength="18" minlength="18" pattern="[0-9]{18}" oninput="this.value = this.value.replace(/[^0-9]/g, '');" placeholder="18 Digit NIP">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email') }}" class="block w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition-colors" required placeholder="Dinkes@puskesmas.com">
                </div>
            ...
            <div class="mt-8 flex gap-4 pt-4 border-t border-slate-100">
                ...
                <button type="submit" class="px-6 py-3 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl transition-colors shadow-lg shadow-teal-500/20">Simpan Data Dinkes</button>
            </div>
        </form>
    </div>
</div>
