...
@section('title', 'Edit Imunisasi Anak')
@section('page_title', 'Edit Imunisasi Anak')
@section('page_subtitle', 'Perbarui data riwayat imunisasi anak')
...
<div class="form-container">
    ...
    <div class="form-card">
        <form method="POST" action="{{ route('bidan.imunisasi.update', $imunisasi) }}">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <select id="id_anak" name="id_anak" required class="form-input" style="appearance: none; background: url('data:image/svg+xml;utf8,<svg viewBox=\"0 0 24 24\" fill=\"%2394A3B8\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M7 10l5 5 5-5z\"/></svg>') no-repeat right 12px center; background-color: #fff; background-size: 24px;">
                    ...
                    @foreach($anakList as $anak)
                        <option value="{{ $anak->id_anak }}" {{ old('id_anak', $imunisasi->id_anak) == $anak->id_anak ? 'selected' : '' }}>
            ...
            <div class="form-group mt-4">
                ...
                <select id="puskesmas_id" name="puskesmas_id" required class="form-input" style="appearance: none; background: url('data:image/svg+xml;utf8,<svg viewBox=\"0 0 24 24\" fill=\"%2394A3B8\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M7 10l5 5 5-5z\"/></svg>') no-repeat right 12px center; background-color: #fff; background-size: 24px;">
                    ...
                    @foreach($puskesmasList as $p)
                        <option value="{{ $p->id }}" {{ old('puskesmas_id', $imunisasi->puskesmas_id) == $p->id ? 'selected' : '' }}>
                            {{ $p->nama_puskesmas }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group mt-4">
                ...
                <select id="posyandu_id" name="posyandu_id" required class="form-input" style="appearance: none; background: url('data:image/svg+xml;utf8,<svg viewBox=\"0 0 24 24\" fill=\"%2394A3B8\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M7 10l5 5 5-5z\"/></svg>') no-repeat right 12px center; background-color: #fff; background-size: 24px;">
                    ...
                    @foreach($posyanduList as $p)
                        <option value="{{ $p->id }}" {{ old('posyandu_id', $imunisasi->posyandu_id) == $p->id ? 'selected' : '' }}>
            ...
            <div class="form-group mt-4">
                ...
                <select id="nama_vaksin" name="nama_vaksin" required class="form-input" style="appearance: none; background: url('data:image/svg+xml;utf8,<svg viewBox=\"0 0 24 24\" fill=\"%2394A3B8\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M7 10l5 5 5-5z\"/></svg>') no-repeat right 12px center; background-color: #fff; background-size: 24px;">
                    ...
                    <option value="Hepatitis B0" {{ old('nama_vaksin', $imunisasi->nama_vaksin) == 'Hepatitis B0' ? 'selected' : '' }}>Hepatitis B0</option>
                    <option value="BCG" {{ old('nama_vaksin', $imunisasi->nama_vaksin) == 'BCG' ? 'selected' : '' }}>BCG</option>
                    <option value="Polio 1" {{ old('nama_vaksin', $imunisasi->nama_vaksin) == 'Polio 1' ? 'selected' : '' }}>Polio 1</option>
                    <option value="Polio 2" {{ old('nama_vaksin', $imunisasi->nama_vaksin) == 'Polio 2' ? 'selected' : '' }}>Polio 2</option>
                    <option value="Polio 3" {{ old('nama_vaksin', $imunisasi->nama_vaksin) == 'Polio 3' ? 'selected' : '' }}>Polio 3</option>
                    <option value="Polio 4" {{ old('nama_vaksin', $imunisasi->nama_vaksin) == 'Polio 4' ? 'selected' : '' }}>Polio 4</option>
                    <option value="DPT-HB-Hib 1" {{ old('nama_vaksin', $imunisasi->nama_vaksin) == 'DPT-HB-Hib 1' ? 'selected' : '' }}>DPT-HB-Hib 1</option>
                    <option value="DPT-HB-Hib 2" {{ old('nama_vaksin', $imunisasi->nama_vaksin) == 'DPT-HB-Hib 2' ? 'selected' : '' }}>DPT-HB-Hib 2</option>
                    <option value="DPT-HB-Hib 3" {{ old('nama_vaksin', $imunisasi->nama_vaksin) == 'DPT-HB-Hib 3' ? 'selected' : '' }}>DPT-HB-Hib 3</option>
                    <option value="Campak / MR" {{ old('nama_vaksin', $imunisasi->nama_vaksin) == 'Campak / MR' ? 'selected' : '' }}>Campak / MR</option>
                </select>
            </div>

            <div class="form-group mt-4">
                <label for="batch_vaksin">Nomor Batch Vaksin <span class="required">*</span></label>
                <input type="text" id="batch_vaksin" name="batch_vaksin" value="{{ old('batch_vaksin', $imunisasi->batch_vaksin) }}" required class="form-input" placeholder="Misal: VAX-2026-X">
            </div>
            ...
            <div class="form-group mt-4">
                ...
                <select id="lokasi_suntikan" name="lokasi_suntikan" required class="form-input" style="appearance: none; background: url('data:image/svg+xml;utf8,<svg viewBox=\"0 0 24 24\" fill=\"%2394A3B8\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M7 10l5 5 5-5z\"/></svg>') no-repeat right 12px center; background-color: #fff; background-size: 24px;">
                    ...
                    <option value="Paha Kiri Atas" {{ old('lokasi_suntikan', $imunisasi->lokasi_suntikan) == 'Paha Kiri Atas' ? 'selected' : '' }}>Paha Kiri Atas</option>
                    <option value="Paha Kanan Atas" {{ old('lokasi_suntikan', $imunisasi->lokasi_suntikan) == 'Paha Kanan Atas' ? 'selected' : '' }}>Paha Kanan Atas</option>
                    <option value="Lengan Kiri" {{ old('lokasi_suntikan', $imunisasi->lokasi_suntikan) == 'Lengan Kiri' ? 'selected' : '' }}>Lengan Kiri</option>
                    <option value="Lengan Kanan" {{ old('lokasi_suntikan', $imunisasi->lokasi_suntikan) == 'Lengan Kanan' ? 'selected' : '' }}>Lengan Kanan</option>
                    <option value="Oral (Mulut)" {{ old('lokasi_suntikan', $imunisasi->lokasi_suntikan) == 'Oral (Mulut)' ? 'selected' : '' }}>Oral (Mulut)</option>
                </select>
            </div>

            <div class="form-group mt-4">
                <label for="suhu_tubuh">Suhu Tubuh Sebelum Vaksin (°C) <span class="required">*</span></label>
                <input type="number" step="0.1" id="suhu_tubuh" name="suhu_tubuh" value="{{ old('suhu_tubuh', $imunisasi->suhu_tubuh) }}" required min="30" max="45" class="form-input" placeholder="Misal: 36.5">
            </div>

            <div class="form-group mt-4">
                <label for="tanggal_pemberian">Tanggal Pemberian <span class="required">*</span></label>
                <input type="date" id="tanggal_pemberian" name="tanggal_pemberian" value="{{ old('tanggal_pemberian', $imunisasi->tanggal_pemberian->format('Y-m-d')) }}" required max="{{ date('Y-m-d') }}" class="form-input">
            </div>

            <div class="form-group mt-4">
                <label for="catatan">Catatan / Edukasi Pasca Vaksin <span class="required">*</span></label>
                <textarea id="catatan" name="catatan" required class="form-input" rows="3" placeholder="Misal: Edukasi jika anak demam, berikan paracetamol...">{{ old('catatan', $imunisasi->catatan) }}</textarea>
            </div>
