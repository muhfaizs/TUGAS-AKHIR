...
@section('page_title', 'Data Pemeriksaan & Tindakan Medis')
@section('page_subtitle', 'Kelola riwayat pemeriksaan kesehatan anak')

@section('content')
<div class="tindakan-header">
    <a href="{{ route('bidan.tindakan.create') }}" class="btn-add">
    ...
    <div class="table-responsive">
        <table class="tindakan-table">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <div class="font-medium text-slate-800">{{ $t->tanggal_pemeriksaan->format('d M Y') }}</div>
                    </td>
                    <td>
                        <div class="font-medium text-slate-800">{{ $t->anak->nama_anak ?? '-' }}</div>
                        <div class="text-xs text-slate-500">NIK: {{ $t->anak->nik_anak ?? '-' }}</div>
                    </td>
                    <td>
                        <div class="text-sm text-slate-600">{{ $t->bidan->nama_lengkap ?? '-' }}</div>
                    </td>
                    <td>
                        <div class="text-sm text-slate-600">{{ $t->diagnosa ?: 'Tidak ada diagnosa' }}</div>
                    </td>
                    <td>
                        <div class="action-buttons">
                            ...
                            <form method="POST" action="{{ route('bidan.anak.send-notification', $t->anak->id_anak) }}" class="inline-form" target="_blank">
                                @csrf
                                <button type="submit" class="btn-icon" style="color: #16A34A;" title="Kirim Hasil via WA & Email" onmouseover="this.style.background='rgba(34,197,94,0.1)';" onmouseout="this.style.background='none';">
                            ...
                            @endif
                            <form method="POST" action="{{ route('bidan.tindakan.destroy', $t) }}" class="inline-form" onsubmit="return confirm('Yakin ingin menghapus data tindakan ini?');">
                                @csrf
                                @method('DELETE')
                ...
                <tr>
                    <td colspan="5" class="text-center py-8">
                        <div class="empty-state">
                            ...
                            <p class="text-slate-500 mt-2">Belum ada data pemeriksaan/tindakan medis.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
