...
@section('page_title', 'Tambah Tindakan Medis')
@section('page_subtitle', 'Catat hasil pemeriksaan atau tindakan medis anak')
...
<div class="form-container">
    ...
    <div class="form-card">
        <form method="POST" action="{{ route('bidan.tindakan.store') }}">
            @csrf
            ...
            <div class="form-group mt-4">
                ...
                <input type="date" id="tanggal_pemeriksaan" name="tanggal_pemeriksaan" value="{{ old('tanggal_pemeriksaan', date('Y-m-d')) }}" required max="{{ date('Y-m-d') }}" class="form-input">
            </div>
            ...
            <div class="form-group mt-4">
                ...
                <input type="text" id="diagnosa" name="diagnosa" value="{{ old('diagnosa') }}" required class="form-input" placeholder="Masukkan diagnosa klinis">
            </div>

            <div class="form-group mt-4">
                <label for="resep_obat">Resep Obat <span class="required">*</span></label>
                <input type="text" id="resep_obat" name="resep_obat" value="{{ old('resep_obat') }}" required class="form-input" placeholder="Masukkan resep obat yang diberikan">
            </div>

            <div class="form-group mt-4">
                <label for="catatan_pemeriksaan">Catatan Pemeriksaan / Tindakan <span class="required">*</span></label>
                <textarea id="catatan_pemeriksaan" name="catatan_pemeriksaan" required class="form-input" rows="4" placeholder="Detail hasil pemeriksaan atau tindakan yang diberikan...">{{ old('catatan_pemeriksaan') }}</textarea>
            </div>
