...
@section('page_title', 'Profil Bidan')
...
<div class="profile-container" style="max-width: 600px; margin: 0 auto; background: #fff; padding: 32px; border-radius: 24px; box-shadow: 0 4px 20px rgba(0,0,0,0.03);">
    ...
    @php
        $oldPuskesmasId = old('puskesmas_id', $user->puskesmas_id);
        $initKabupatenId = '';
        if ($oldPuskesmasId) {
            $pusk = $puskesmasList->firstWhere('id', $oldPuskesmasId);
            if ($pusk) {
    ...
    <form method="POST" action="{{ route('bidan.profile.update') }}" x-data="{ 
        showPassword: false, 
        ...
        init() {
            this.$nextTick(() => {
                this.puskesmas_id = {{ $oldPuskesmasId ?: '""' }};
            });
        },
        ...
        <div style="display: flex; flex-direction: column; gap: 20px;">
            ...
            <div>
                ...
                <select id="kabupaten_id" x-model="kabupaten_id" class="form-input" style="width: 100%; padding: 12px 16px; border: 1px solid #CBD5E1; border-radius: 12px; background-color: #fff;" @change="puskesmas_id = ''">
            ...
            <!-- Puskesmas -->
            <div>
                <label for="puskesmas_id" style="display: block; font-size: 13px; font-weight: 600; color: #475569; margin-bottom: 8px;">Puskesmas (Tempat Tugas) <span style="color: #EF4444;">*</span></label>
                <select name="puskesmas_id" id="puskesmas_id" x-model="puskesmas_id" required class="form-input" style="width: 100%; padding: 12px 16px; border: 1px solid #CBD5E1; border-radius: 12px; background-color: #fff;">
