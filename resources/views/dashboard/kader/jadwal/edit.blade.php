...
@section('page_subtitle', 'Perbarui detail jadwal kegiatan posyandu')
...
<div style="background: #fff; border-radius: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.03); padding: 32px; max-width: 800px; margin: 0 auto;">
    <form action="{{ route('kader.jadwal.update', $jadwal->id_jadwal) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 24px;">
            <div class="form-group" style="grid-column: 1 / -1;">
                ...
                <input type="date" id="tanggal" name="tanggal" value="{{ old('tanggal', $jadwal->tanggal->format('Y-m-d')) }}" required class="form-input" style="width: 100%; padding: 12px 16px; border: 1px solid #E2E8F0; border-radius: 12px; outline: none; transition: border-color 0.2s;">
                @error('tanggal')
            ...
            <div class="form-group">
                ...
                <input type="time" id="waktu_mulai" name="waktu_mulai" value="{{ old('waktu_mulai', \Carbon\Carbon::parse($jadwal->waktu_mulai)->format('H:i')) }}" required class="form-input" style="width: 100%; padding: 12px 16px; border: 1px solid #E2E8F0; border-radius: 12px; outline: none;">
                @error('waktu_mulai')
                    <span style="color: #EF4444; font-size: 12px; margin-top: 4px; display: block;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="time" id="waktu_selesai" name="waktu_selesai" value="{{ old('waktu_selesai', \Carbon\Carbon::parse($jadwal->waktu_selesai)->format('H:i')) }}" required class="form-input" style="width: 100%; padding: 12px 16px; border: 1px solid #E2E8F0; border-radius: 12px; outline: none;">
                @error('waktu_selesai')
                    <span style="color: #EF4444; font-size: 12px; margin-top: 4px; display: block;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group" style="grid-column: 1 / -1;">
                ...
                <select id="lokasi" name="lokasi" required class="form-input" style="width: 100%; padding: 12px 16px; border: 1px solid #E2E8F0; border-radius: 12px; outline: none; font-family: inherit; appearance: none; background: url('data:image/svg+xml;utf8,<svg viewBox=\&quot;0 0 24 24\&quot; fill=\&quot;%2394A3B8\&quot; xmlns=\&quot;http://www.w3.org/2000/svg\&quot;><path d=\&quot;M7 10l5 5 5-5z\&quot;/></svg>') no-repeat right 12px center; background-color: #fff; background-size: 24px;">
                    <option value="{{ $posyanduName }}" {{ old('lokasi', $jadwal->lokasi) == $posyanduName ? 'selected' : '' }}>{{ $posyanduName }} (Default)</option>
                    <option value="Balai Desa / Balai RW" {{ old('lokasi', $jadwal->lokasi) == 'Balai Desa / Balai RW' ? 'selected' : '' }}>Balai Desa / Balai RW</option>
                    <option value="Puskesmas Pembantu" {{ old('lokasi', $jadwal->lokasi) == 'Puskesmas Pembantu' ? 'selected' : '' }}>Puskesmas Pembantu</option>
                    <option value="Rumah Kader / Ketua RT" {{ old('lokasi', $jadwal->lokasi) == 'Rumah Kader / Ketua RT' ? 'selected' : '' }}>Rumah Kader / Ketua RT</option>
                    @if(!in_array(old('lokasi', $jadwal->lokasi), [$posyanduName, 'Balai Desa / Balai RW', 'Puskesmas Pembantu', 'Rumah Kader / Ketua RT']))
                        <option value="{{ old('lokasi', $jadwal->lokasi) }}" selected>{{ old('lokasi', $jadwal->lokasi) }}</option>
                    @endif
                </select>
                @error('lokasi')
                    <span style="color: #EF4444; font-size: 12px; margin-top: 4px; display: block;">{{ $message }}</span>
            ...
            <div class="form-group" style="grid-column: 1 / -1;">
                ...
                <textarea id="keterangan" name="keterangan" rows="3" class="form-input" style="width: 100%; padding: 12px 16px; border: 1px solid #E2E8F0; border-radius: 12px; outline: none; font-family: inherit;">{{ old('keterangan', $jadwal->keterangan) }}</textarea>
                @error('keterangan')
                    <span style="color: #EF4444; font-size: 12px; margin-top: 4px; display: block;">{{ $message }}</span>
                @enderror
            </div>
