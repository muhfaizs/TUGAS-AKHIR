...
@section('header_subtitle', 'Data pemeriksaan Terpadu')

@section('content')
<div class="mb-6 flex items-center justify-between">
    <div>
        <h2 class="text-2xl font-bold text-slate-800">Pasien: {{ $anc->ibuHamil->nama_lengkap }}</h2>
        <p class="text-slate-500">Tanggal Pemeriksaan: {{ \Carbon\Carbon::parse($anc->tanggal_pemeriksaan)->translatedFormat('d F Y') }}</p>
    </div>
    <a href="{{ route('ibu-hamil.show', $anc->ibu_hamil_id) }}" class="text-teal-600 hover:text-teal-700 font-medium text-sm flex items-center gap-1">
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 border-l-4 border-l-amber-500">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <p class="text-slate-500 text-sm mb-1">Temu Wicara / Konseling:</p>
                    <div class="bg-slate-50 p-3 rounded-xl border border-slate-200 text-sm font-medium">
                        {{ $anc->konseling ?: '-' }}
                    </div>
                </div>
