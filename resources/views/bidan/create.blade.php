...
@section('title', 'Tambah Bidan - SatuKIA')
@section('header_title', 'Tambah Bidan Baru')
@section('header_subtitle', 'Masukkan data pengguna bidan ke dalam sistem')

@section('content')
<div class="max-w-3xl">
    <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
        <form action="{{ route('bidan.store') }}" method="POST" class="p-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" class="block w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition-colors" required placeholder="Masukkan nama lengkap bidan">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email') }}" class="block w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition-colors" required placeholder="bidan@puskesmas.com">
                </div>
            ...
            <div class="mt-8 flex gap-4 pt-4 border-t border-slate-100">
                ...
                <button type="submit" class="px-6 py-3 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl transition-colors shadow-lg shadow-teal-500/20">Simpan Data Bidan</button>
            </div>
        </form>
    </div>
</div>
