    ...
    <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
        <div class="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100">
            ...
            <div class="flex gap-3">
                <a href="{{ route('bidan.create') }}" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-xl transition-colors shadow-sm shadow-teal-500/20">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd" />
                    </svg>
                    Tambah Bidan
                </a>
                <a href="{{ route('dinkes.create') }}" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-lime-500 hover:bg-lime-600 text-slate-900 font-semibold rounded-xl transition-colors shadow-sm shadow-lime-500/20">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd" />
                    </svg>
                    Tambah Dinkes
                </a>
                <a href="{{ route('ortu.create') }}" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-teal-50 hover:bg-teal-100 text-teal-700 font-semibold rounded-xl transition-colors border border-teal-100">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd" />
                    </svg>
                    Tambah Ibu Hamil
        ...
        <div class="p-6 md:p-8 pt-4">
            <table id="bidan-table" class="w-full text-left border-collapse">
                <thead>
                    <tr>
                    ...
                    <tr class="hover:bg-slate-50 transition-colors group">
                        <td class="py-4 border-b border-slate-50">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-full @if($pengguna->role === 'bidan') bg-teal-100 text-teal-700 @elseif($pengguna->role === 'dinkes') bg-purple-100 text-purple-700 @else bg-indigo-100 text-indigo-700 @endif flex items-center justify-center font-bold">
                                    {{ substr($pengguna->name, 0, 2) }}
                                </div>
                                <div>
                                    <p class="font-bold text-slate-800">{{ $pengguna->name }}</p>
                                    <p class="text-xs text-slate-500">{{ $pengguna->email }}</p>
                                    <p class="text-[10px] text-slate-400 mt-0.5">{{ in_array($pengguna->role, ['bidan', 'dinkes']) ? 'NIP: ' . ($pengguna->nip ?? '-') : 'NIK: ' . ($pengguna->nik ?? '-') }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="py-4 border-b border-slate-50">
                            @if($pengguna->role === 'bidan')
                                <span class="px-2.5 py-1 rounded-lg text-xs font-bold bg-teal-50 text-teal-700 border border-teal-100">Bidan</span>
                            @elseif($pengguna->role === 'dinkes')
                                <span class="px-2.5 py-1 rounded-lg text-xs font-bold bg-purple-50 text-purple-700 border border-purple-100">Dinkes</span>
                            @else
                        ...
                        <td class="py-4 border-b border-slate-50 text-right">
                            <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ $pengguna->role === 'bidan' ? route('bidan.edit', $pengguna->id) : ($pengguna->role === 'dinkes' ? route('dinkes.edit', $pengguna->id) : route('ortu.edit', $pengguna->id)) }}" class="p-2 text-blue-500 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                      <path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                    </svg>
                                </a>
                                <form action="{{ $pengguna->role === 'bidan' ? route('bidan.destroy', $pengguna->id) : ($pengguna->role === 'dinkes' ? route('dinkes.destroy', $pengguna->id) : route('ortu.destroy', $pengguna->id)) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pengguna ini?');">
                                    @csrf
                                    @method('DELETE')
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        const table = new simpleDatatables.DataTable("#bidan-table", {
            searchable: true,
            fixedHeight: true,
            perPage: 10,
            labels: {
