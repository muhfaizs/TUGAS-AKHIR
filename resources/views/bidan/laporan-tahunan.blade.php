...
@section('content')
    ...
    <div class="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-100 mb-8">
        <form action="{{ route('bidan.laporan-tahunan') }}" method="GET" class="flex flex-col md:flex-row md:items-end gap-6">
            <div class="flex-1">
    ...
    <div class="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-100">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('bidan.laporan-export', ['type' => 'tahunan', 'format' => 'pdf', 'tahun' => request('tahun', date('Y'))]) }}" class="px-5 py-2.5 bg-slate-800 hover:bg-slate-900 text-white text-sm font-semibold rounded-xl transition-colors flex items-center gap-2 shadow-sm">
                @else
                ...
                @if($ibuHamils->count() > 0)
                <a href="{{ route('bidan.laporan-export', ['type' => 'tahunan', 'format' => 'excel', 'tahun' => request('tahun', date('Y'))]) }}" class="px-5 py-2.5 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-xl transition-colors flex items-center gap-2 shadow-sm shadow-green-500/20">
                @else
                ...
                <form action="{{ route('bidan.laporan-kirim') }}" method="POST" class="m-0">
                    @csrf
                    <input type="hidden" name="periode" value="Tahun {{ request('tahun', date('Y')) }}">
